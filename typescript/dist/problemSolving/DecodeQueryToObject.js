...
const test = (parent, arr) => {
    let obj = {};
    let parentObj = (parent && parent[arr[0]]) ? parent[arr[0]] : {};
    if (arr.length < 3)
        obj[arr[0]] = decodeURIComponent(arr[1]);
    else
        obj[arr[0]] = Object.assign(Object.assign({}, parentObj), test(parentObj[arr[0]], arr.slice(1)));
    return obj;
};
function convertQueryToMap(query) {
    ...
    arr.forEach(e => {
        ...
        if (key_values.length == 2)
            obj[key_values[0]] = decodeURIComponent(key_values[1]);
        else
            obj[key_values[0]] = Object.assign(Object.assign({}, obj[key_values[0]]), test(obj[key_values[0]], key_values.slice(1)));
    });
    return obj;
}
